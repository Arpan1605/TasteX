...
public sealed class KitchenController(IKitchenDashboardContract service) : ApiControllerBase
{
    ...
    [HttpPatch("orders/{orderId:long}/payment")]
    public async Task<IActionResult> UpdatePayment([FromRoute] long orderId, [FromBody] JsonElement body, CancellationToken cancellationToken)
    {
        static bool TryParsePaymentStatus(JsonElement body, out PaymentStatus status)
        {
            status = default;
            ...
            {
                if (Enum.IsDefined(typeof(PaymentStatus), number))
                {
            ...
            if (node.ValueKind == JsonValueKind.String)
            {
                ...
                if (int.TryParse(raw, out var asNumber) && Enum.IsDefined(typeof(PaymentStatus), asNumber))
                {
                    status = (PaymentStatus)asNumber;
                    return true;
                }
        ...
        if (!TryParsePaymentStatus(body, out var newStatus))
        {
            return BadRequest(new ApiResponse<UpdatePaymentStatusResponse>(false, null, new ApiError("PAYMENT_STATUS_INVALID", "Invalid or missing 'newStatus'.")));
        }
        ...
        return ToActionResult(await service.UpdatePaymentStatusAsync(request, cancellationToken));
    }
